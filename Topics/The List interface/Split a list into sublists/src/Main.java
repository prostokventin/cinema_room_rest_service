...
class ListUtils {
    ...
    public static <T> List<List<T>> splitListIntoSubLists(List<T> list, int subListSize) {
        ...
        for (int i = 0; i < list.size(); i+=subListSize) {
            sublists.add(list.subList(i, (i + subListSize) < list.size() ? (i + subListSize) : list.size()));

//            System.out.println(list.subList(i, (i + subListSize) < list.size() ? (i + subListSize) : list.size()));
        }
